# -*- coding: utf-8 -*-
"""
risk_output.json + cluster 리포트(+선택 events) -> 소설톤 JSON (확정적 기술 시나리오)
- 출력 스키마(엄수):
{
  "LLM 응답": [
    {
      ...
      "위험도점수": 0.0,                 # 0~10
      "추정정확도": 0.0,                 # 0.0~1.0 (모델의 판단 확신도)
      "영향범위": ["...","..."],          # 자산/데이터 범위
      "근거": [
        {"시간":"ISO8601","출처":"risk|cluster|event","요약":"한 줄","참조":{"cluster_id":"...","event_id":"..."}}
      ],
      "권장대응": ["1. ...","2. ...","3. ..."]  # 번호 포함 문자열 배열
    }
  ]
}

실행 예)
python run_llm5.py --risk risk_output.json --cluster "message (1).txt" \
  --events events.json --out story_output.json \
  --backend gemini --model "gemini-1.5-flash" --temperature 0.1
"""
...
def _summarize_events(events_json: dict, max_items: int = 120) -> List[Dict[str, Any]]:
...
def _validate_response_json(obj: dict) -> None:
...
def main():
    ...
    default_events = "sample log2.txt"       # 선택
    default_out = "story_output.json"    # JSON으로 저장
    ...
    ap.add_argument("--temperature", type=float, default=0.1)  # 확정적 톤 유지
    ...
    events_summary = _summarize_events(events_json, max_items=120) if events_json else []
    ...
    if args.backend == "gemini":
        fallback_models = [args.model, "gemini-1.5-flash", "gemini-1.5-flash-8b"]
