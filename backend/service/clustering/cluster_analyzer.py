...
class ClusterAnalyzer:
    ...
    def analyze_cluster(self, events: List[SecurityEvent]) -> ClusterMetrics:
        ...
        # 가중합
        risk = (time_conc*self.weights['time'] + ip_div*self.weights['ip'] +
                user_anom*self.weights['user'] + file_sens*self.weights['file'])

        # 다축 게이팅
        axes = sum([1 if time_conc>0.6 else 0,
    ...
    def _generate_attack_scenario(self, events: List[SecurityEvent], m: Dict[str, float]) -> str:
        s = []
        if m['user_anomaly'] > 0.7: s.append("관리자 탈취/자격남용 의심")
        if m['time_concentration'] > 0.6: s.append("단시간 집중 활동")
        if m['file_sensitivity'] > 0.6: s.append("민감/DB 대량 접근")
        if m['ip_diversification'] > 0.6: s.append("내부 연쇄/다중 IP 활용")
        return " + ".join(s) if s else "일반적 이벤트"
